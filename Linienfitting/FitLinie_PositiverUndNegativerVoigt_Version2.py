...
Bei der grafischen Auswahl der Linie muss man 2 mal mit der Maus in den plot
klicken, zuerst links der Linie (mit möglichst viel Kontinuum), dann rechts
(mit möglichst viel Kontinuum). Dadurch werden die betreffenden Wellenlängen 
und Flüsse ausgewählt.
...
for k in np.arange(len(filelist)):
    ...
    extremum1 = x[y.argmax()]
    extremum2 = x[y.argmin()]
...
# Modelldefinitionen:
    ...
    # Doppelvoigt: sigma und amplitude anpassen !!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    if graphspeichern == 'y':
        plt.savefig('Linefit_Voigt_'+filelist[k] + '.pdf')

plt.show()


# Gefittete Modellparameter als Excel-Datei im Arbeitsverzeichnis abspeichern
ergebnis.to_excel('Linefit_Voigt_Ergebnisse.xlsx', sheet_name='Fitergebnisse')
