...
Verschiedene Modelle (Gauss, Voigt, SplitLorentz) können ausgewählt werden.
...
Bei der grafischen Auswahl der Linie muss man dreimal mit der Maus in den plot
klicken, zuerst links der Linie (mit ausreichend Kontinuum), dann rechts und als
drittes das ungefähre Linienminimum/maximum. Dadurch werden die betreffenden
Wellenlängen ausgewählt.

Nach welchem Modell gefittet werden soll, wird gewählt. Zur Auswahl stehen
Gauss, Doppelgauss, Lorentz, Voigt, Doppelvoigt.
Die Ergebnisse des Fittings werden in einer Excel-Datei gespeichert.

Bei Emissionslinien sollte in den Anfangswerten für die Modelle die Amplitude
positiv gewählt werden. Bei Absorptionslinien negativ. Bei Doppelpeaks (blends
zweier Linien) muss der Abstand zwischen den Peaks in den Anfangswerten der
Modelle angepasst werden, damit der Erstfit (=initial fit im linken Teil der
erzeugten Grafik) etwa zu dem gemessenen Linienprofil passt:
in den Zeilen center=dict(value=extremum-10) die Zahl variieren.
...
frage1 = input(
    "Möchten Sie die Spektrumgrenzen und das Linienminimum/maximum zahlenmäßig \
eingeben (m) oder per Mausklick (grafisch, g)? Geben Sie m oder g ein: "
)

if frage1 == "g":
    # Interaktives Festlegen der Wellenlängengrenzen
    # linke Seite, dann rechte Seite, dann das Minimum/Maximum der Linie anklicken.
    pts = []
    ...
    extremum = pts[2, 0]
    print("Gewählter Bereich:", begin, " bis", end)
    print()
...
if frage1 == "m":
    ...
    print()
    extremum = float(
        input("Geben Sie die ungefähre Wellenlänge des Linienminimums/maximums ein: "))
...
# Modelldefinitionen:
# Bei Emissionslinien sollte in den Anfangswerten für die Modelle die Amplitude
# positiv gewählt werden. Bei Absorptionslinien negativ.
